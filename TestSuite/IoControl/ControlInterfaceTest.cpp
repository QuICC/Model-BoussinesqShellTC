/** \file ControlInterfaceTest.cpp
 *  \brief Implementation of test case for ControlInterface
 */

#include "gtest/gtest.h"

namespace GeoMHDiSCC {

namespace TestSuite {

   /**
    * @brief Test fixture for the ControlInterface implementation
    */
   class ControlInterfaceTest : public ::testing::Test {
      public:

      protected:
         /**
          * @brief Constructor
          */
         ControlInterfaceTest();

         /**
          * @brief Destructor
          */
         virtual ~ControlInterfaceTest();

         /**
          * @brief Do Set-up work before each test
          */
         //virtual void SetUp() {};

         /**
          * @brief Do tear-down work after each test
          */
         //virtual void TearDown() {};
   };

   ControlInterfaceTest::ControlInterfaceTest()
   {
   }

   ControlInterfaceTest::~ControlInterfaceTest()
   {
   }

//   void ControlInterfaceTest::SetUp()
//   {
//   }

//   void ControlInterfaceTest::TearDown()
//   {
//   }

   /**
    * @brief Test default constructor
    */
   TEST_F(ControlInterfaceTest, Constructor) {
      ASSERT_TRUE(false) << "##########################################" << std::endl << "## Tests have not yet been implemented! ##" << std::endl << "##########################################";
   }

}
}

/// Main to execute all test from test case
int main(int argc, char **argv) {
   ::testing::InitGoogleTest(&argc, argv);
   return RUN_ALL_TESTS();
}
