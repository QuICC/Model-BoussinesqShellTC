...
class MumpsLU: internal::noncopyable
{
  ...
  public:
    ...
    inline const internal::solve_retval<MumpsLU, Rhs> solve(const MatrixBase<Rhs>& b) const
    {
    ...
    /** \returns the solution x of \f$ A x = b \f$ using the current decomposition of A.
      ...
      */
    template<typename Rhs>
    inline const internal::sparse_solve_retval<MumpsLU, Rhs> solve(const SparseMatrixBase<Rhs>& b) const
    {
      eigen_assert(m_isInitialized && "MumpsLU is not initialized.");
      eigen_assert(rows()==b.rows()
                && "MumpsLU::solve(): invalid number of rows of the right hand side matrix b");
