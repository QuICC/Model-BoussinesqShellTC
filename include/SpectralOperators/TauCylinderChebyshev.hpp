/** 
 * @file TauCylinderChebyshev.hpp
 * @brief Implementation of the Tau conditions for chebyshev based operators for a cylinder radius
 * @author Philippe Marti \<philippe.marti@colorado.edu\>
 */

#ifndef TAUCYLINDERCHEBYSHEV_HPP
#define TAUCYLINDERCHEBYSHEV_HPP
...
namespace Spectral {
    ...
    * @brief Implementation of the Tau conditions for chebyshev based operators for a cylinder radius
    */
   class TauCylinderChebyshev: public ITauBoundary
   {
      public:
         /**
          * @brief Constructor
          ...
          */
         TauCylinderChebyshev(const MHDFloat c, const int nN, const Boundary::BCVector& bcs, const int nEq);

         /**
          * @brief Empty Destructor
          */
         ~TauCylinderChebyshev();
   ...
   template <typename TData> inline const TData& TauCylinderChebyshev::extend(const TData& spec)
   {
      return spec;
   }

   template <typename TData> inline const TData& TauCylinderChebyshev::restrict(const TData& spec)
   {
      return spec;
   }

   template <typename TData> void TauCylinderChebyshev::constrainKronBlock(TData& mat)
   {
   }

   template <typename TData> bool TauCylinderChebyshev::constrainKronProduct(TData& mat)
   {
   ...
   template <> inline bool TauCylinderChebyshev::constrainKronProduct<SparseMatrixZ>(SparseMatrixZ& mat)
   {
      if(this->mIsComplex)
      {
         mat =  this->mZTau;
   ...
   template <> inline bool TauCylinderChebyshev::constrainKronProduct<DecoupledZSparse>(DecoupledZSparse& mat)
   {
      if(this->mIsComplex)
      {
   ...
   template <> inline bool TauCylinderChebyshev::constrainKronProduct<SparseMatrix>(SparseMatrix& mat)
   {
      if(this->mIsComplex)
      {
...
#endif // TAUCYLINDERCHEBYSHEV_HPP
