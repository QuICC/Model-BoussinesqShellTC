...
namespace Transform {
    ...
    * @brief This class defines the backward single grouping exchange grouping algorithm for the second exchange
    */
   template <typename TConfigurator> class BackwardSingle2DGrouper: public BackwardGrouperBase
   {
      public:
         /**
          * @brief Constructor
   ...
   template <typename TConfigurator> ArrayI BackwardSingle2DGrouper<TConfigurator>::packs1D(const std::map<PhysicalNames::Id, std::pair<bool,TriBool> >& varInfo)
   {
      return this->namePacks1D(varInfo);
   }

   template <typename TConfigurator> ArrayI BackwardSingle2DGrouper<TConfigurator>::packs2D(const std::map<PhysicalNames::Id, std::pair<bool,TriBool> >& varInfo)
   {
