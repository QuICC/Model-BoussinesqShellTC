...
namespace Transform {
    ...
    * @brief This class defines the forward transform grouping algorithm
    */
   template <typename TConfigurator> class ForwardTransformGrouper: public ForwardGrouperBase
   {
      public:
         /**
          * @brief Constructor
   ...
   template <typename TConfigurator> inline void ForwardTransformGrouper<TConfigurator>::transform(std::vector<SharedIScalarEquation>& scalEqs, std::vector<SharedIVectorEquation>& vectEqs, TransformCoordinatorType& coord)
   {
      //
   ...
   template <typename TConfigurator> ArrayI ForwardTransformGrouper<TConfigurator>::packs1D(const std::map<PhysicalNames::Id, std::pair<bool,TriBool> >& varInfo)
   {
   ...
   template <typename TConfigurator> ArrayI ForwardTransformGrouper<TConfigurator>::packs2D(const std::map<PhysicalNames::Id, std::pair<bool,TriBool> >& varInfo)
   {  
