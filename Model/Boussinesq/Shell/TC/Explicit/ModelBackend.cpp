            ...
            rowId == colId)
   {
                       ...
                       const NonDimensional::NdMap& nds)
      {
         ...
         if (l > 0)
         {
            ...
            {
               SparseSM::Chebyshev::LinearMap::I4Y4SphLapl2 spasm(nNr, nNc, ri,
                  ro, l);
               bMat = spasm.mat();
            }
         }
                       ...
                       const NonDimensional::NdMap& nds)
      {
         ...
         if (l > 0)
         {
            ...
            else
            {
               ...
               SparseSM::Chebyshev::LinearMap::I4Y4SphLapl spasm(nNr, nNc, ri,
                  ro, l);
...
void ModelBackend::splitBoundaryValueBlock(DecoupledZSparse& decMat,
   const SpectralFieldId& fieldId, const int matIdx, const Resolution& res,
   const std::vector<MHDFloat>& eigs, const NonDimensional::NdMap& nds) const
{
   assert(eigs.size() == 1);
      ...
      for (auto pRowId = imRange.first; pRowId != imRange.second; pRowId++)
      {
         ...
         for (auto pColId = imRange.first; pColId != imRange.second; pColId++)
         {
            ...
            auto descr = splitBoundaryValueBlockBuilder(rowId, colId, res, eigs,
               bcs, nds);
            buildFixedBlock(rModelMatrix, 2, true, descr, rowId, colId, fields, matIdx,
   ...
   const NonDimensional::NdMap& nds) const
{
   ...
   if (opId == ModelOperator::ExplicitLinear::id())
   {
      ...
      buildBlock(decMat, descr, rowId, colId, fields, matIdx, bcType, res, l, l,
         bcs, nds, false, true);
   }
