...
namespace Timestep {
   ...
   void TimestepCoordinator::computeRHS()
   {
      ...
      {
         // Compute linear solve RHS
         (*solRIt)->computeRHS(this->mStep);
      }
   }

   void TimestepCoordinator::buildSolverMatrix(TimestepCoordinator::SharedRSolverType spSolver, const int matIdx, Equations::SharedIEquation spEq, FieldComponents::Spectral::Id comp, const int idx)
   {
      this->buildSolverMatrixWrapper(spSolver, matIdx, spEq, comp, idx);
   }

   void TimestepCoordinator::buildSolverMatrix(TimestepCoordinator::SharedZSolverType spSolver, const int matIdx, Equations::SharedIEquation spEq, FieldComponents::Spectral::Id comp, const int idx)
   {
      this->buildSolverMatrixWrapper(spSolver, matIdx, spEq, comp, idx);
   }
