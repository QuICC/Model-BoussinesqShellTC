/** 
 * @file SHmTools.cpp
 * @brief Source of the Spherical Harmonics tools for spatial schemes with m spectral ordering
...
namespace Schemes {
   ...
   int SHmTools::nM(const int l, const int nM)
   {
      return std::min(l+1, nM);
   }

   int SHmTools::nHarmonics(const int nL, const int nM)
   {
      int tot = 0;
      ...
      for(int l = 0; l < nL; l++)
      {
         ...
         for(int m = 0; m < SHmTools::nM(l, nM); m++)
         {
            tot++;
         }
      }
   ...
   void SHmTools::buildLMap(std::multimap<int,int>& harmonics, const int nL, const int nM)
   {
   ...
   void SHmTools::buildLHMap(std::multimap<int,int>& harmonics, const int nL, const int nM, const int h0, const int nH)
   {
      ...
      for(int l=0; l < maxL/2; ++l)
      {
         ...
         for(int m=0; m < SHmTools::nM(maxL-l-1, nM); ++m)
         {
   ...
   void SHmTools::initMLLoad(std::deque<int>& list, std::vector<int>& load, std::queue<int>& optimal, const int nL, const int nM, const int bins)
   {
   ...
   void SHmTools::combineMPairs(std::deque<int>& list, std::multimap<int, int>& regular, const int nM, const int bins)
   {
   ...
   void SHmTools::fillMPairsBins(std::deque<int>& list, std::multimap<int, int>& regular, std::vector<int>& load, const int bins)
   {
   ...
   void SHmTools::fillMRestBins(std::deque<int>& list, std::multimap<int, int>& regular, std::vector<int>& load, std::queue<int>& optimal, const int bins)
   {
      // Obviously only required if any load is left
      if(list.size() > 0)
      {
   ...
   void SHmTools::convertLoadToOrders(std::multimap<int, int>& regular, const int nL)
   {
