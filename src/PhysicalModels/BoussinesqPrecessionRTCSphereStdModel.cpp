/** 
 * @file BoussinesqPrecessionRTCSphereStdModel.cpp
 * @brief Source of the Boussinesq combined precession and rotating thermal convection in a sphere (Toroidal/Poloidal formulation) without coupled solve (standard implementation)
...
// Class include
//
#include "PhysicalModels/BoussinesqPrecessionRTCSphereStdModel.hpp"
...
#include "Equations/Sphere/Boussinesq/BoussinesqPrecessionRTCSphereTransport.hpp"
#include "Equations/Sphere/Boussinesq/BoussinesqPrecessionRTCSphereMomentum.hpp"
...
namespace QuICC {
   ...
   const std::string BoussinesqPrecessionRTCSphereStdModel::PYMODULE = "boussinesq_precessionrtcsphere_std";

   const std::string BoussinesqPrecessionRTCSphereStdModel::PYCLASS = "BoussinesqPrecessionRTCSphereStd";

   void BoussinesqPrecessionRTCSphereStdModel::addEquations(SharedSimulation spSim)
   {
      // Add transport equation
      spSim->addScalarEquation<Equations::BoussinesqPrecessionRTCSphereTransport>();
      
      // Add Navier-Stokes equation
      spSim->addVectorEquation<Equations::BoussinesqPrecessionRTCSphereMomentum>();
   }

   void BoussinesqPrecessionRTCSphereStdModel::addStates(SharedStateGenerator spGen)
   {
   ...
   void BoussinesqPrecessionRTCSphereStdModel::addVisualizers(SharedVisualizationGenerator spVis)
   {
   ...
   void BoussinesqPrecessionRTCSphereStdModel::setVisualizationState(SharedVisualizationGenerator spVis)
   {
   ...
   void BoussinesqPrecessionRTCSphereStdModel::addAsciiOutputFiles(SharedSimulation spSim)
   {
   ...
   void BoussinesqPrecessionRTCSphereStdModel::addHdf5OutputFiles(SharedSimulation spSim)
   {
      // Field IDs iterator
   ...
   void BoussinesqPrecessionRTCSphereStdModel::addStatsOutputFiles(SharedSimulation spSim)
   {
   }

   void BoussinesqPrecessionRTCSphereStdModel::setInitialState(SharedSimulation spSim)
   {
      // Field IDs iterator
      std::vector<PhysicalNames::Id>::const_iterator  it;
      std::vector<PhysicalNames::Id> ids = PhysicalModelBase::fieldIds();
