/** 
 * @file BoussinesqPrecessionRTCDynamoSphereStdModel.cpp
 * @brief Source of the Boussinesq combined precession and thermal convection dynamo in a sphere (Toroidal/Poloidal formulation) without coupled solve (standard implementation)
...
// Class include
//
#include "PhysicalModels/BoussinesqPrecessionRTCDynamoSphereStdModel.hpp"
...
#include "Equations/Sphere/Boussinesq/BoussinesqPrecessionRTCDynamoSphereTransport.hpp"
#include "Equations/Sphere/Boussinesq/BoussinesqPrecessionRTCDynamoSphereMomentum.hpp"
...
namespace QuICC {
   ...
   const std::string BoussinesqPrecessionRTCDynamoSphereStdModel::PYMODULE = "boussinesq_precessionrtcdynamosphere_std";

   const std::string BoussinesqPrecessionRTCDynamoSphereStdModel::PYCLASS = "BoussinesqPrecessionRTCDynamoSphereStd";

   void BoussinesqPrecessionRTCDynamoSphereStdModel::addEquations(SharedSimulation spSim)
   {
      // Add transport equation
      spSim->addScalarEquation<Equations::BoussinesqPrecessionRTCDynamoSphereTransport>();
      
      // Add Navier-Stokes equation
      spSim->addVectorEquation<Equations::BoussinesqPrecessionRTCDynamoSphereMomentum>();
      
      // Add induction equation
      spSim->addVectorEquation<Equations::BoussinesqPrecessionRTCDynamoSphereInduction>();
   }

   void BoussinesqPrecessionRTCDynamoSphereStdModel::addStates(SharedStateGenerator spGen)
   {
   ...
   void BoussinesqPrecessionRTCDynamoSphereStdModel::addVisualizers(SharedVisualizationGenerator spVis)
   {
   ...
   void BoussinesqPrecessionRTCDynamoSphereStdModel::setVisualizationState(SharedVisualizationGenerator spVis)
   {
   ...
   void BoussinesqPrecessionRTCDynamoSphereStdModel::addAsciiOutputFiles(SharedSimulation spSim)
   {
   ...
   void BoussinesqPrecessionRTCDynamoSphereStdModel::addHdf5OutputFiles(SharedSimulation spSim)
   {
      // Field IDs iterator
   ...
   void BoussinesqPrecessionRTCDynamoSphereStdModel::addStatsOutputFiles(SharedSimulation spSim)
   {
   }

   void BoussinesqPrecessionRTCDynamoSphereStdModel::setInitialState(SharedSimulation spSim)
   {
      // Field IDs iterator
      std::vector<PhysicalNames::Id>::const_iterator  it;
      std::vector<PhysicalNames::Id> ids = PhysicalModelBase::fieldIds();
