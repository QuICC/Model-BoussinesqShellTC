...
namespace IoVariable {
   ...
   Cartesian1DMagneticEnergyWriter::Cartesian1DMagneticEnergyWriter(const std::string& prefix, const std::string& type, const bool hasZonalX, const bool hasZonalY)
      : IVariableAsciiWriter(prefix + EnergyTags::BASENAME, EnergyTags::EXTENSION, prefix + EnergyTags::HEADER, type, EnergyTags::VERSION, Dimensions::Space::SPECTRAL, IVariableAsciiWriter::EXTEND), mHasZonalX(hasZonalX), mHasZonalY(hasZonalY), mXEnergy(-1.0), mYEnergy(-1.0), mZEnergy(-1.0), mXZonalXEnergy(-1.0), mZZonalXEnergy(-1.0), mYZonalYEnergy(-1.0), mZZonalYEnergy(-1.0)
   {
   }
   ...
   void Cartesian1DMagneticEnergyWriter::computeEnergy(Transform::TransformCoordinatorType& coord, const Cartesian1DMagneticEnergyWriter::EnergyTypeId flag)
   {
      ...
      *pZ = 0.0;
      ...
      sIt = sRange.first;
      sIt++;
      if(sRange.first->first == PhysicalNames::BX)
      {
      ...
      } else if(sRange.first->first == PhysicalNames::BY)
      {
