...
namespace IoVariable {
   ...
   void ShellTorPolDissipationWriter::compute(Transform::TransformCoordinatorType& coord)
   {
      ...
      // Compute projection transform for first dimension 
      coord.transform1D().project(rOutVarTorQ.rData(), rInVarTorQ.data(), Transform::TransformCoordinatorType::Transform1DType::ProjectorType::PROJ);

      // Compute |f|^2
      rOutVarTorQ.rData() = rOutVarTorQ.rData().array()*rOutVarTorQ.rData().conjugate().array();

      // Compute projection transform for first dimension 
      coord.transform1D().integrate_full(rInVarTorQ.rData(), rOutVarTorQ.data(), Transform::TransformCoordinatorType::Transform1DType::IntegratorType::INTG);
      ...
      // Compute Diss in Q component of QST decomposition (toroidal component)
         ...
         for(int k = 0; k < this->mspRes->cpu()->dim(Dimensions::Transform::TRA1D)->dim<Dimensions::Data::DAT3D>(); ++k)
         {
            ...
            if( firstM == 0)
            {
               MHDFloat ModeDiss = lfactor*(this->mIntgOp*rInVarTorQ.slice(k).col(0).real())(0);
               this->mTorDiss += ModeDiss;
               start = 1;
               firstM = 1;
               /*
      ...
      // Compute projection transform for first dimension 
      coord.transform1D().project(rOutVarTorS.rData(), rInVarTorS.data(), Transform::TransformCoordinatorType::Transform1DType::ProjectorType::DIFFR);

      // Compute |f|^2
      rOutVarTorS.rData() = rOutVarTorS.rData().array()*rOutVarTorS.rData().conjugate().array();

      // Compute projection transform for first dimension 
      coord.transform1D().integrate_full(rInVarTorS.rData(), rOutVarTorS.data(), Transform::TransformCoordinatorType::Transform1DType::IntegratorType::INTG);
      ...
      // Compute Diss in S component of QST decomposition
         ...
         for(int k = 0; k < this->mspRes->cpu()->dim(Dimensions::Transform::TRA1D)->dim<Dimensions::Data::DAT3D>(); ++k)
         {
            ...
            if(firstM == 0)
            {
               MHDFloat ModeDiss =  lfactor*(this->mIntgOp*rInVarTorS.slice(k).col(0).real())(0);
               this->mTorDiss += ModeDiss;
               start = 1;
               firstM = 1;
