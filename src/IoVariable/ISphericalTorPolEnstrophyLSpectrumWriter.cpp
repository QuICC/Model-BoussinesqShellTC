 ...
 * @brief Source of the implementation of the ASCII spherical harmonics spectral enstrophy calculation for a vector field in a sphere
 */
...
namespace IoVariable {
   ...
   ISphericalTorPolEnstrophyLSpectrumWriter::ISphericalTorPolEnstrophyLSpectrumWriter(const std::string& prefix, const std::string& type)
      : ISphericalTorPolEnstrophyBaseWriter(prefix + EnstrophySpectrumTags::LBASENAME, EnstrophySpectrumTags::EXTENSION, prefix + EnstrophySpectrumTags::HEADER, type, EnstrophySpectrumTags::VERSION, Dimensions::Space::SPECTRAL, OVERWRITE), mTorEnstrophy(0), mPolEnstrophy(0)
   {
   }
   ...
   void ISphericalTorPolEnstrophyLSpectrumWriter::storePEnstrophy(const int l, const int m, const MHDFloat enstrophy)
   {
      this->mPolEnstrophy(l) += enstrophy; 
   }

   void ISphericalTorPolEnstrophyLSpectrumWriter::storeT1Enstrophy(const int l, const int m, const MHDFloat enstrophy)
   {
      this->mTorEnstrophy(l) += enstrophy; 
   }

   void ISphericalTorPolEnstrophyLSpectrumWriter::storeT2Enstrophy(const int l, const int m, const MHDFloat enstrophy)
   {
      this->mTorEnstrophy(l) += enstrophy; 
   }

   void ISphericalTorPolEnstrophyLSpectrumWriter::write()
   {
      ...
      if(FrameworkMacro::allowsIO())
      {
         ...
         this->mFile << std::left << std::setw(ioIW) << "# l" << "\t";
