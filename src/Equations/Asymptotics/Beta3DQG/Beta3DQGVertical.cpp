...
namespace Equations {
   ...
   Beta3DQGVertical::Beta3DQGVertical(SharedEquationParameters spEqParams)
   ...
   {
      StreamAdvection::set(rNLComp, this->scalar(PhysicalNames::STREAMFUNCTION).dom(0).grad(), this->unknown().dom(0).grad(), 1.0);
   }

   void Beta3DQGVertical::setRequirements()
   {
      // Set vertical velocity as equation unknown
      this->mName = PhysicalNames::VELOCITYZ;
   ...
   {
      // Set field coupling
         // .. set coupling to the streamfunction 
   ...
   void Beta3DQGVertical::timestepInput(FieldComponents::Spectral::Component id, DecoupledZMatrix& storage, const int matIdx, const int start)
   {
   ...
   void Beta3DQGVertical::timestepInput(FieldComponents::Spectral::Component id, MatrixZ& storage, const int matIdx, const int start)
   {
      // Copy field values into timestep input
      this->copyTInput(id,storage,matIdx,start);
   ...
   void Beta3DQGVertical::timestepOutput(FieldComponents::Spectral::Component id, const DecoupledZMatrix& storage, const int matIdx, const int start)
   {
      // Copy timestep output into field value
      this->copyTOutput(id, storage, matIdx, start);
   }

   void Beta3DQGVertical::timestepOutput(FieldComponents::Spectral::Component id, const MatrixZ& storage, const int matIdx, const int start)
   {
      // Copy timestep output into field value
      this->copyTOutput(id, storage, matIdx, start);
   }
   ...
   void Beta3DQGVertical::setSpectralMatrices(Code::SpectralOperator1DType& spec1D, Code::SpectralOperator2DType& spec2D, Code::SpectralOperator3DType& spec3D)
   {
      // Simplify notation
      ...
      tau3D = spec3D.tau(this->getBCs(FieldComponents::Spectral::NONE,Dimensions::THREED), atTop);
      ...
      tau3D.second = std::tan((MathConstants::PI/180.)*this->eqParams().nd(ParameterNames::NonDimensional::CHI))*spec3D.tau(this->getCBCs(FieldComponents::Spectral::NONE,Dimensions::THREED), atTop).second;
