...
namespace Equations {
   ...
   void BoussinesqFPlane3DQGTransport::computeNonlinear(Datatypes::PhysicalScalarType& rNLComp, FieldComponents::Physical::Id id) const
   {
      ...
      rNLComp.setData((2.0*(-eta3*this->scalar(PhysicalNames::VELOCITYZ).dom(0).phys().data().array() + eta2*this->unknown().dom(0).grad().comp(FieldComponents::Physical::TWO).data().array())*this->scalar(PhysicalNames::MEANTEMPERATURE).dom(0).grad().comp(FieldComponents::Physical::ONE).data().array()).matrix());
   }
