...
namespace GeoMHDiSCC {
   ...
   ScalarEquation::ScalarEquation(SharedEquationParameters spEqParams)
      : EvolutionEquation(spEqParams)
   {
   }

   void ScalarEquation::applyQuasiInverse(FieldComponents::Spectral::Component id, DecoupledZMatrix& storage, const int matIdx, const int start)
   {
   ...
   void ScalarEquation::applyQuasiInverse(FieldComponents::Spectral::Component id, MatrixZ& storage, const int matIdx, const int start)
   {
      int rows = 2*this->unknown().dom(0).perturbation().slice(matIdx).rows()/3;
      int cols = this->unknown().dom(0).perturbation().slice(matIdx).cols();
   ...
   void ScalarEquation::copyTInput(FieldComponents::Spectral::Component id, DecoupledZMatrix& storage, const int matIdx, const int start)
   {
      int rows = 2*this->unknown().dom(0).perturbation().slice(matIdx).rows()/3;
      int cols = this->unknown().dom(0).perturbation().slice(matIdx).cols();
   ...
   void ScalarEquation::copyTInput(FieldComponents::Spectral::Component id, MatrixZ& storage, const int matIdx, const int start)
   {
      int rows = 2*this->unknown().dom(0).perturbation().slice(matIdx).rows()/3;
      int cols = this->unknown().dom(0).perturbation().slice(matIdx).cols();
   ...
   void ScalarEquation::copyTOutput(FieldComponents::Spectral::Component id, const DecoupledZMatrix& storage, const int matIdx, const int start)
   {
      int rows = 2*this->unknown().dom(0).perturbation().slice(matIdx).rows()/3;
      int cols = this->unknown().dom(0).perturbation().slice(matIdx).cols();
   ...
   void ScalarEquation::copyTOutput(FieldComponents::Spectral::Component id, const MatrixZ& storage, const int matIdx, const int start)
   {
      int rows = 2*this->unknown().dom(0).perturbation().slice(matIdx).rows()/3;
      int cols = this->unknown().dom(0).perturbation().slice(matIdx).cols();
