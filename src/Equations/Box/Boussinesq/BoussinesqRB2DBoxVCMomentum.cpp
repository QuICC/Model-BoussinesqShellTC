...
namespace Equations {
   ...
   void BoussinesqRB2DBoxVCMomentum::computeNonlinear(Datatypes::PhysicalScalarType& rNLComp, FieldComponents::Physical::Id id) const
   {
      ...
      ///   \f$ \left(\vec u\cdot\nabla\right)u_x\f$
      ///
      Physical::VelocityAdvection<FieldComponents::Physical::X,FieldComponents::Physical::Y,FieldComponents::Physical::Z>::set(rNLComp.comp(FieldComponents::Physical::Z), this->unknown().dom(0).phys(), this->unknown().dom(0).phys().comp(FieldComponents::Physical::Z), this->unknown().dom(0).phys().comp(FieldComponents::Physical::Z), this->unknown().dom(0).grad(id), 1.0);
   }
