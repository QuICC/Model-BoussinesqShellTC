/** 
 * @file BoussinesqRRBCDuctVCTransport.cpp
 * @brief Source of the implementation of the transport equation in rotating Rayleigh-Benard convection in a infinite duct
...
namespace Equations {
   ...
   BoussinesqRRBCDuctVCTransport::BoussinesqRRBCDuctVCTransport(SharedEquationParameters spEqParams)
   ...
   BoussinesqRRBCDuctVCTransport::~BoussinesqRRBCDuctVCTransport()
   {
   }

   void BoussinesqRRBCDuctVCTransport::setCoupling()
   {
      this->defineCoupling(FieldComponents::Spectral::SCALAR, CouplingInformation::PROGNOSTIC, 0, true, false);
   }

   void BoussinesqRRBCDuctVCTransport::computeNonlinear(Datatypes::PhysicalScalarType& rNLComp, FieldComponents::Physical::Id id) const
   {
   ...
   void BoussinesqRRBCDuctVCTransport::setRequirements()
   {
