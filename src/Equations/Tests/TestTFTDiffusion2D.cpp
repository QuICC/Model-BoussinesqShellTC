...
namespace Equations {
   ...
   TestTFTDiffusion2D::TestTFTDiffusion2D(SharedEquationParameters spEqParams)
   ...
   DecoupledZSparse TestTFTDiffusion2D::operatorRow(const IEquation::OperatorRowId opId, FieldComponents::Spectral::Id compId, const int matIdx, const bool hasBoundary) const
   {
   ...
   void linearBlock(const TestTFTDiffusion2D& eq, FieldComponents::Spectral::Id compId, DecoupledZSparse& mat, const SpectralFieldId fieldId, const std::vector<MHDFloat>& eigs, const bool hasBoundary)
   {
      assert(eigs.size() == 1);
