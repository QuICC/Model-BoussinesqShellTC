/** 
 * @file TestFFFBackwardScalar.cpp
 * @brief Source of the implementation of a test equation for the FFF scheme with exact known scalar spectral space solution
...
namespace Equations {
   ...
   TestFFFBackwardScalar::TestFFFBackwardScalar()
   ...
   void TestFFFBackwardScalar::setIdentity(const PhysicalNames::Id name)
   {
      // Set the name
      this->setName(name);
   ...
   void TestFFFBackwardScalar::setSolutionType(const TestFFFBackwardScalar::SolutionTypeId id)
   {
      this->mTypeId = id;
   }

   void TestFFFBackwardScalar::setCoupling()
   {
      // Get X dimension
   ...
   void TestFFFBackwardScalar::setRequirements()
   {
   ...
   void TestFFFBackwardScalar::init()
   {
   ...
   MHDFloat TestFFFBackwardScalar::scalarPoint(const int i, const int j, const int k) const
   {
   ...
   void TestFFFBackwardScalar::computeNonlinear(Datatypes::PhysicalScalarType& rNLComp, FieldComponents::Physical::Id id) const
   {
   ...
   MHDFloat TestFFFBackwardScalar::computeScalarError() const
   {
      // Maximum error
      MHDFloat error = 0.0;
