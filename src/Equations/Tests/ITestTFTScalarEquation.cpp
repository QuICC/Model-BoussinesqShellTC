/** \file ITestTFTScalarEquation.cpp
 *  \brief Source of the implementation of the general scalar equation for the TFT scheme test
 */
...
namespace Equations {
   ...
   ITestTFTScalarEquation::~ITestTFTScalarEquation()
   {
   }

   void ITestTFTScalarEquation::setCoupling()
   {
      // Get X dimension
      ...
      TestTFTSystem::setCouplingInfo(this->mCouplingInfos.find(FieldComponents::Spectral::SCALAR)->second, eqId, nX, nZ, nY);
   }

   DecoupledZSparse ITestTFTScalarEquation::linearRow(FieldComponents::Spectral::Id comp, const int matIdx) const
   {
      ...
      for(fIt = fRange.first; fIt != fRange.second; ++fIt)
      {
         ...
         SpectralFieldId eqId = std::make_pair(this->name(), FieldComponents::Spectral::SCALAR);
         TestTFTSystem::linearBlock(block, eqId, *fIt, nx, nz, k_, Ra, Pr, Gamma, chi);
   ...
   DecoupledZSparse ITestTFTScalarEquation::timeRow(FieldComponents::Spectral::Id comp, const int matIdx) const
   {
      // Get X and Z dimensions
      int nx = this->unknown().dom(0).spRes()->sim()->dim(Dimensions::Simulation::SIM1D, Dimensions::Space::SPECTRAL);
      int nz = this->unknown().dom(0).spRes()->sim()->dim(Dimensions::Simulation::SIM3D, Dimensions::Space::SPECTRAL);
      ...
      SpectralFieldId eqId = std::make_pair(this->name(), FieldComponents::Spectral::SCALAR);
      TestTFTSystem::timeBlock(block, eqId, nx, nz, k_, Ra, Pr, Gamma, chi);
      Eigen::kroneckerProduct(blockMatrix, block.first, tmp);
      matrixRow.first += tmp;
      Eigen::kroneckerProduct(blockMatrix, block.second, tmp);
      matrixRow.second += tmp;
   ...
   DecoupledZSparse ITestTFTScalarEquation::boundaryRow(FieldComponents::Spectral::Id comp, const int matIdx) const
   {
      // Get X and Z dimensions
      int nx = this->unknown().dom(0).spRes()->sim()->dim(Dimensions::Simulation::SIM1D, Dimensions::Space::SPECTRAL);
      int nz = this->unknown().dom(0).spRes()->sim()->dim(Dimensions::Simulation::SIM3D, Dimensions::Space::SPECTRAL);
      ...
      for(fIt = fRange.first; fIt != fRange.second; ++fIt)
      {
         ...
         SpectralFieldId eqId = std::make_pair(this->name(), FieldComponents::Spectral::SCALAR);
         TestTFTSystem::boundaryBlock(block, eqId, *fIt, this->mspBcIds, nx, nz, k_, Ra, Pr, Gamma, chi);
         Eigen::kroneckerProduct(blockMatrix, block.first, tmp);
         matrixRow.first += tmp;
         Eigen::kroneckerProduct(blockMatrix, block.second, tmp);
         matrixRow.second += tmp;
   ...
   void ITestTFTScalarEquation::initSpectralMatrices(const SharedSimulationBoundary spBcIds)
   {
      // Equation key
      SpectralFieldId eqId = std::make_pair(this->name(), FieldComponents::Spectral::SCALAR);
      ...
      for(int i = 0; i < nSystems; ++i)
      {
         ...
         TestTFTSystem::quasiInverse(qIt->second.back(), eqId, nx, nz);
      }
      ...
      for(fIt = fRange.first; fIt != fRange.second; ++fIt)
      {
         ...
         for(int i = 0; i < nSystems; ++i)
         {
            ...
            TestTFTSystem::linearBlock(tmpMat.at(i), eqId, *fIt, nx, nz, k_, Ra, Pr, Gamma, chi);
