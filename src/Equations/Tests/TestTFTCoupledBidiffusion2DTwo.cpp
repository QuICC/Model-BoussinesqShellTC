...
namespace Equations {
   ...
   TestTFTCoupledBidiffusion2DTwo::TestTFTCoupledBidiffusion2DTwo(SharedEquationParameters spEqParams)
      : IScalarEquation(spEqParams), mSecondName(PhysicalNames::MAGNETIC)
   {
   }

   TestTFTCoupledBidiffusion2DTwo::~TestTFTCoupledBidiffusion2DTwo()
   {
   }

   void TestTFTCoupledBidiffusion2DTwo::setIdentity(const PhysicalNames::Id first, const PhysicalNames::Id second)
   {
      // Set the name
      this->setName(first);
   ...
   void TestTFTCoupledBidiffusion2DTwo::setRequirements()
   {
   ...
   DecoupledZSparse TestTFTCoupledBidiffusion2DTwo::operatorRow(const IEquation::OperatorRowId opId, FieldComponents::Spectral::Id compId, const int matIdx) const
   {
   ...
   void linearBlock(const TestTFTCoupledBidiffusion2DTwo& eq, FieldComponents::Spectral::Id compId, DecoupledZSparse& mat, const SpectralFieldId fieldId, const std::vector<MHDFloat>& eigs)
   {
      assert(eigs.size() == 1);
   ...
   void timeBlock(const TestTFTCoupledBidiffusion2DTwo& eq, FieldComponents::Spectral::Id compId, DecoupledZSparse& mat, const std::vector<MHDFloat>& eigs)
   {
      assert(eigs.size() == 1);
      MHDFloat k = eigs.at(0);
   ...
   void boundaryBlock(const TestTFTCoupledBidiffusion2DTwo& eq, FieldComponents::Spectral::Id compId, DecoupledZSparse& mat, const SpectralFieldId fieldId, const std::vector<MHDFloat>& eigs)
   {
      assert(eigs.size() == 1);
      MHDFloat k = eigs.at(0);
