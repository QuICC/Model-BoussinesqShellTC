...
namespace Equations {
   ...
   void TestTTTDiffusion2D::createBoundaries(FieldComponents::Spectral::Id compId, const int matIdx)
   {
      //EigenSelector::boundaryRow(*this, compId, matIdx);
   }

   DecoupledZSparse TestTTTDiffusion2D::operatorRow(const IEquation::OperatorRowId opId, FieldComponents::Spectral::Id compId, const int matIdx) const
   {
   ...
   void linearBlock(const TestTTTDiffusion2D& eq, FieldComponents::Spectral::Id compId, DecoupledZSparse& mat, const SpectralFieldId fieldId, const std::vector<MHDFloat>& eigs)
   {
      assert(eigs.size() == 1);
   ...
   void timeBlock(const TestTTTDiffusion2D& eq, FieldComponents::Spectral::Id compId, DecoupledZSparse& mat, const SpectralFieldId fieldId, const std::vector<MHDFloat>& eigs)
   {
      assert(eigs.size() == 1);
      MHDFloat k = eigs.at(0);
