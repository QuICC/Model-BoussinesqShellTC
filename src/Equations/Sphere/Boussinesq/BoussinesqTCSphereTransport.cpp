/** 
 * @file BoussinesqTCSphereTransport.cpp
...
namespace Equations {
   ...
   BoussinesqTCSphereTransport::BoussinesqTCSphereTransport(SharedEquationParameters spEqParams)
   ...
   BoussinesqTCSphereTransport::~BoussinesqTCSphereTransport()
   {
   }

   void BoussinesqTCSphereTransport::setCoupling()
   {
      this->defineCoupling(FieldComponents::Spectral::SCALAR, CouplingInformation::PROGNOSTIC, 0, true, false);
   }

   void BoussinesqTCSphereTransport::computeNonlinear(Datatypes::PhysicalScalarType& rNLComp, FieldComponents::Physical::Id id) const
   {
   ...
   void BoussinesqTCSphereTransport::setRequirements()
   {
