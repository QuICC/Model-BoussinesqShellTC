...
namespace GeoMHDiSCC {
   ...
   EvolutionEquation::EvolutionEquation(SharedEquationParameters spEqParams)
      : mspEqParams(spEqParams), mBCCounter(-1), mEqIsComplex(false)
   {
   }

   bool EvolutionEquation::isComplex(FieldComponents::Spectral::Component id) const
   {
      return (this->mEqIsComplex || this->boundaryIsComplex(id));
   }

   bool EvolutionEquation::boundaryIsComplex(FieldComponents::Spectral::Component id) const
   {
      bool status = false;
   ...
   int EvolutionEquation::nBC(FieldComponents::Spectral::Component id, Dimensions::Type dim) const
   {
      int n = 0;
   ...
   void EvolutionEquation::addBC(FieldComponents::Spectral::Component id, Dimensions::Type dim, const std::pair<BoundaryConditions::Id,BoundaryConditions::Position>& bc, ArrayZ val)
   {
   ...
   void EvolutionEquation::addCBC(FieldComponents::Spectral::Component id, Dimensions::Type dim, const std::pair<BoundaryConditions::Id,BoundaryConditions::Position>& bc)
   {
      std::pair<FieldComponents::Spectral::Component,Dimensions::Type> myId = std::make_pair(id,dim);
   ...
   void EvolutionEquation::init()
   {
      this->setCoupling();
   }

   bool EvolutionEquation::isInitialised() const
   {
      bool status;
   ...
   bool EvolutionEquation::initialisedBCs() const
   {
   ...
   int EvolutionEquation::rowShift(FieldComponents::Spectral::Component id, const int j) const
   {
   ...
   void EvolutionEquation::finalizeMatrices()
   {
      ...
      for(mapIt = this->mTMatrices.begin(); mapIt != this->mTMatrices.end(); ++mapIt)
      {
         ...
         {
            throw Exception("EvolutionEquation::finalizeMatrices", "Incompatible opertor and boundary matrices vectors");
         }
