/** 
 * @file BoussinesqRBCBoxVCModel.cpp
 * @brief Source of the Boussinesq Rayleigh-Benard convection in 3D box (velocity-continuity formulation) model
...
// Class include
//
#include "PhysicalModels/BoussinesqRBCBoxVCModel.hpp"
...
#include "Equations/Box/Boussinesq/BoussinesqRBCBoxVCMomentum.hpp"
#include "Equations/Box/Boussinesq/BoussinesqRBCBoxVCContinuity.hpp"
...
namespace QuICC {
   ...
   const std::string BoussinesqRBCBoxVCModel::PYMODULE = "boussinesq_rbcbox_vc";

   const std::string BoussinesqRBCBoxVCModel::PYCLASS = "BoussinesqRBCBoxVC";

   void BoussinesqRBCBoxVCModel::addEquations(SharedSimulation spSim)
   {
      // Add transport equation
      spSim->addScalarEquation<Equations::BoussinesqRBCBoxVCTransport>();
      
      // Add Navier-Stokes equation (X,Y,Z components)
      spSim->addVectorEquation<Equations::BoussinesqRBCBoxVCMomentum>();

      // Add continuity equation
      spSim->addScalarEquation<Equations::BoussinesqRBCBoxVCContinuity>();
   }

   void BoussinesqRBCBoxVCModel::addStates(SharedStateGenerator spGen)
   {
   ...
   void BoussinesqRBCBoxVCModel::addVisualizers(SharedVisualizationGenerator spVis)
   {
   ...
   void BoussinesqRBCBoxVCModel::setVisualizationState(SharedVisualizationGenerator spVis)
   {
   ...
   void BoussinesqRBCBoxVCModel::addAsciiOutputFiles(SharedSimulation spSim)
   {
   ...
   void BoussinesqRBCBoxVCModel::addStatsOutputFiles(SharedSimulation spSim)
   {
   }
