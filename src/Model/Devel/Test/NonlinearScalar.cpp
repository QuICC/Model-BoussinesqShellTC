...
namespace Test {
   ...
   NonlinearScalar::NonlinearScalar(SharedEquationParameters spEqParams)
      : IScalarEquation(spEqParams)
   {
   }

   NonlinearScalar::~NonlinearScalar()
   {
   }

   void NonlinearScalar::setIdentity(const PhysicalNames::Id name)
   {
      // Set the name
      this->setName(name);
   ...
   void NonlinearScalar::computeNonlinear(Datatypes::PhysicalScalarType& rNLComp, FieldComponents::Physical::Id id) const
   {
   ...
   Datatypes::SpectralScalarType::PointType NonlinearScalar::sourceTerm(FieldComponents::Spectral::Id compId, const int iX, const int iZ, const int iY) const
   {
      // Assert on scalar component is used
