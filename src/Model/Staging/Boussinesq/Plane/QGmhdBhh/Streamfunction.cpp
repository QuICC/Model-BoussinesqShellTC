...
namespace QGmhdBhh {
   ...
   void Streamfunction::computeNonlinear(Datatypes::PhysicalScalarType& rNLComp, FieldComponents::Physical::Id id) const
   {
     ...
     /// nonlinear Lorentz force: \left(B_\perp)\cdot \nabla b_\perp
      ...
      /// nonlinear Lorentz force: \left(b_\perp)\cdot \nabla b_\perp
      rNLComp.addData((Q/MPR)*(-this->scalar(PhysicalNames::FBX).dom(0).phys().data().array()*(this->scalar(PhysicalNames::FBY).dom(0).grad2().comp(FieldComponents::Physical::X,FieldComponents::Physical::X).data().array() - this->scalar(PhysicalNames::FBX).dom(0).grad2().comp(FieldComponents::Physical::X,FieldComponents::Physical::Y).data().array()) - this->scalar(PhysicalNames::FBY).dom(0).phys().data().array()*(this->scalar(PhysicalNames::FBY).dom(0).grad2().comp(FieldComponents::Physical::X,FieldComponents::Physical::Y).data().array() - this->scalar(PhysicalNames::FBX).dom(0).grad2().comp(FieldComponents::Physical::Y,FieldComponents::Physical::Y).data().array())).matrix());
   }
