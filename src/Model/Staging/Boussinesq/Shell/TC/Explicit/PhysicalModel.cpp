...
// Class include
//
#include MAKE_STR( QUICC_MODEL_PATH/Boussinesq/Shell/TC/Explicit/PhysicalModel.hpp )

// Project includes
//
#include MAKE_STR( QUICC_MODEL_PATH/Boussinesq/Shell/TC/Transport.hpp )
...
namespace Explicit {
   ...
   const std::string PhysicalModel::PYMODULE = "boussinesq_tcshell_std";

   const std::string PhysicalModel::PYCLASS = "BoussinesqTCShellStd";

   void PhysicalModel::addEquations(SharedSimulation spSim)
   {
      // Add transport equation
      spSim->addScalarEquation<Equations::Boussinesq::Shell::TC::Transport>();
                                                           
      // Add Navier-Stokes equation                        
      spSim->addVectorEquation<Equations::Boussinesq::Shell::TC::Momentum>();
   }
   ...
   void PhysicalModel::addStates(SharedStateGenerator spGen)
   {
      ...
      if(true)
      {
         ...
         tSH.clear(); 
         tSH.push_back(std::make_tuple(0,0,MHDComplex(1,1)));
         tSH.push_back(std::make_tuple(1,0,MHDComplex(1,1)));
         tSH.push_back(std::make_tuple(1,1,MHDComplex(1,1)));
         tSH.push_back(std::make_tuple(2,0,MHDComplex(1,1)));
         tSH.push_back(std::make_tuple(2,1,MHDComplex(1,1)));
         tSH.push_back(std::make_tuple(2,2,MHDComplex(1,1)));
         tSH.push_back(std::make_tuple(5,5,MHDComplex(1,1)));
