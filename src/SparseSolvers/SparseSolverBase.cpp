...
namespace Solver {
   ...
   void SparseSolverBase::initStartRow()
   {
      ...
      int i = 0;
      for(std::map<SpectralFieldId, std::pair<int, ArrayI> >::iterator jIt = this->mInformation.begin(); jIt != this->mInformation.end(); ++jIt)
      {
         for(std::map<SpectralFieldId, std::pair<int, ArrayI> >::iterator iIt = this->mInformation.begin(); iIt != this->mInformation.end(); ++iIt)
         {  
