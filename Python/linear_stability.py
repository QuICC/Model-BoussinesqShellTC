...
def sptarn(A, B, lb, ub, tolconv = 100*np.spacing(1), jmax = 100, maxmul = 10):
   ...
   while ntries < maxtries:
      ...
      v = F(t)
      fact = nplin.norm(v,np.inf)*nplin.norm(C.todense(), np.inf, None)
   ...
   for mul in range(maxmul):
      ...
      if US.shape[1] > 0:
         v = np.matrix(np.hstack((US, ortho_gs(vstart, US)[0])))
      else:
         v = np.matrix(vstart)

      if T.shape[1] > 0:
         h = np.matrix(np.vstack((T, np.zeros((1,nt)))))
      else:
         h = np.matrix(np.zeros((1,nt)))
      convrun = 0
      ...
      for j in range(nt+1,jmax+1):
         ...
         h = np.vstack((h, np.hstack((np.zeros((1,j)), hj[j]))))
         ...
         if j == jt:
            ...
            tc = splin.rsf1csf(ts, us)[0]
            lms = shift + 1/np.diag(tc)
            sij = np.abs((h[j+1,j]/normhj)*us[j-nt,:].conj().T)

            convoutside = np.logical_and(np.logical_or(lms <= lb, lms > ub) , sij <= tolstab)
            nonconvinside = np.logical_and(np.logical_and(lb < lmb, lms <= ub), np.logical_and(sij > tol, np.abs(us[1,:]).conj().T > tolspur))
            convrun = np.logical_or(convrun , np.logical_and(np.any(convoutside) and not np.any(nonconvinside)))

            if np.any(nonconvinside):
               incinside = np.nonzero(nonconvinside)
               iintr = incinside[1]
            else:
               iintr = 1
      ...
      if nc > 0:
         T = np.vstack((np.hstack((T, h[0:nt,iv]*us[:,0:nc-1])), np.hstack((np.zeros((nc,nt)), ts[0:nc,0:nc-1]))))
         US = np.hstack(US, v[:,iv]*us[:,0:nc-1])
         nt = nt + nc
         if nt >= jmax:
            terminatealg = True
      ...
      if nc > 0 and np.any(np.logical_and(newinside ,sij <= tol)):
         ...
         vstart = v[:,iv]*us[:,np.nonzero(newinside)[0]]
      else:
         terminatealg = True
   ...
   s, dt = nplin.eig(np.multiply(T,(np.abs(T)>tol*normhj)),'nobalance')
   ev = shift + 1/np.diag(dt)

   inside = np.nonzero(np.logical_and(lb < ev ,ev >= ub))
   iresult = len(inside)
   ievsrt = np.argsort(ev[inside])
   lmb = ev[ievsrt]
