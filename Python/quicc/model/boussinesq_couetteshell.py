...
class BoussinesqCouetteShellOld(BoussinesqCouetteShellConfig, base_model.BaseModel):
    ...
    def implicit_block(self, res, eq_params, eigs, bcs, field_row, field_col, restriction=None):
        ...
        if field_row == ("velocity", "tor"):
            if field_col == ("velocity", "tor"):
                mat = geo.i2r2lapl(res[0], res[1], m, a, b, bc, E, with_sh_coeff='laplh', l_zero_fix='zero',
            ...
            if field_col == ("velocity", "tor"):
                ...
                mat = geo.i4r4lapl2(res[0], res[1], m, a, b, bc, E, with_sh_coeff='laplh', l_zero_fix='zero',
                                    restriction=restriction)
                bc[0] = min(bc[0], 0)
                mat = mat + geo.i4r4lapl(res[0], res[1], m, a, b, bc, 1j * m, l_zero_fix='zero',
                                         restriction=restriction)
    ...
    def time_block(self, res, eq_params, eigs, bcs, field_row, restriction=None):
        ...
        bc = self.convert_bc(eq_params, eigs, bcs, field_row, field_row)
        if field_row == ("velocity", "tor"):
            mat = geo.i2r2(res[0], res[1], m, a, b, bc, with_sh_coeff='laplh', l_zero_fix='set',
                           restriction=restriction)

        elif field_row == ("velocity", "pol"):
            mat = geo.i4r4lapl(res[0], res[1], m, a, b, bc, with_sh_coeff='laplh', l_zero_fix='set',
                               restriction=restriction)
...
class BoussinesqCouetteShellVisuVisuOld(BoussinesqCouetteShellConfig, base_model.BaseModel):
    """Class to setup the Boussinesq spherical Couette in a spherical shell (Toroidal/Poloidal formulation) without field coupling (standard implementation)"""
