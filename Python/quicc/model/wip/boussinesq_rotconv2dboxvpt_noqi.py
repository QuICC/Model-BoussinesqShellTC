"""Module provides the functions to generate the Boussinesq convection in a rotating 2D box model (velocity-pressure-temperature) without quasi-inverse"""
...
class BoussinesqRotConv2DBoxVPTNoQI(base_model.BaseModel):
    """Class to setup the Boussinesq convection in a rotating 2D box model (velocity-pressure-temperature) without quasi-inverse"""
    ...
    def linear_block(self, res, eq_params, eigs, bcs, field_row, field_col):
        ...
        if field_row == ("velocityx",""):
            if field_col == ("velocityx",""):
                mat = c2d.lapl(res[0], res[2], 0, bc)

            elif field_col == ("velocityz",""):
                mat = c2d.zblk(res[0], res[2], 2, 2, bc, location = 'b')

            elif field_col == ("pressure",""):
                bc['x']['cr'] = pN
                bc['z']['cr'] = pN
                mat = c2d.d1d0(res[0], res[2], bc, -1.0, sx = 2, sz = 2)
            ...
            if field_col == ("velocityx",""):
                ...
                bc['x']['cr'] = pN
                bc['z']['cr'] = pN
                mat = c2d.d0d1(res[0], res[2], bc, -1.0, sx = 2, sz = 2)

            elif field_col == ("temperature",""):
                mat = c2d.sid(res[0], res[2], 2, 2, bc, Ra/16.0)
            ...
            if field_col == ("velocityx",""):
                ...
                mat = c2d.sid(res[0], res[2], 2, 2, bc)
            ...
            if field_col == ("velocityx",""):
                ...
                mat = c2d.lapl(res[0]-pN, res[2]-pN, 0, bc, -1.0)

            elif field_col == ("temperature",""):
                bc['x']['rb'] = pN
                bc['z']['rb'] = pN
                mat = c2d.d0d1(res[0], res[2], bc, Ra/16.0, sx = 2, sz = 2)
    ...
    def time_block(self, res, eq_params, eigs, bcs, field_row):
        ...
        bc = self.convert_bc(eq_params,eigs,bcs,field_row,field_row)
        if field_row == ("velocityx",""):
            mat = c2d.sid(res[0], res[2], 2, 2, bc, 1.0/Pr)

        elif field_row == ("velocityz",""):
            mat = c2d.sid(res[0], res[2], 2, 2, bc, 1.0/Pr)

        elif field_row == ("pressure",""):
            mat = c2d.zblk(res[0]-pN, res[2]-pN, 2, 2, bc, location = 'b')
