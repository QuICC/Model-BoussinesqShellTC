"""Module provides the functions to generate the Boussinesq rotating thermal dynamo in a plane layer (2 periodic directions) (Toroidal/Poloidal formulation)"""
...
class BoussinesqDynamoPlaneConfig:
    """Class to setup the Boussinesq rotating therml dynamo in a plane layer (2 periodic directions) (Toroidal/Poloidal formulation)"""
    ...
    def nondimensional_parameters(self):
        ...
        return ["magnetic_prandtl", "prandtl", "rayleigh", "ekman", "scale1d", "fast_mean", "rescaled"]
    ...
    def config_fields(self):
        ...
        return ["magnetic", "velocity", "temperature"]
...
class BoussinesqDynamoPlane(BoussinesqDynamoPlaneConfig, base_model.BaseModel):
    ...
    def implicit_fields(self, field_row):
        ...
        if field_row[0] in ["velocity", "temperature"]:
            fields =  [("velocity","tor"), ("velocity","pol"), ("temperature","")]
        else:
            fields = [field_row]
    ...
    def block_size(self, res, eigs, bcs, field_row):
        ...
        if self.use_galerkin:
            if field_row in [("magnetic","tor"), ("magnetic","pol"), ("velocity","tor"), ("temperature","")]:
                shift_z = 2
    ...
    def implicit_block(self, res, eq_params, eigs, bcs, field_row, field_col, restriction = None):
        ...
        if field_row == ("velocity","tor") and kx == 0 and ky == 0:
            if field_col == ("velocity","tor"):
                mat = geo.i2d2(res[0], bc, Pm, cscale = zscale)

            elif field_col == ("velocity","pol"):
                mat = geo.i2(res[0], bc, Pm*A**2/E)
        ...
        elif field_row == ("magnetic","tor") and kx == 0 and ky == 0:
            if field_col == ("magnetic","tor"):
                mat = geo.i2d2(res[0], bc, 1.0, cscale = zscale)

        # Mean Y magnetic field
        elif field_row == ("magnetic","pol") and kx == 0 and ky == 0:
        ...
        # Toroidal velocity
            ...
            if field_col == ("velocity","tor"):
                mat = geo.i2(res[0], bc, Pm*(kx**2 + ky**2)**2)
                bc[0] = min(bc[0], 0)
                mat += geo.i2d2(res[0], bc, -Pm*(kx**2 + ky**2), cscale = zscale)

            elif field_col == ("velocity","pol"):
                mat = geo.i2d1(res[0], bc, -Pm*(kx**2 + ky**2)*A**2/E, cscale = zscale)
        ...
        # Poloidal velocity
            ...
            if field_col == ("velocity","tor"):
                mat = geo.i4d1(res[0], bc, Pm*(kx**2 + ky**2)*A**2/E, cscale = zscale)

            elif field_col == ("velocity","pol"):
                mat = geo.i4(res[0], bc, -Pm*(kx**2 + ky**2)**3)
                bc[0] = min(bc[0], 0)
                mat += geo.i4d2(res[0], bc, Pm*2.0*(kx**2 + ky**2)**2, cscale = zscale)
                mat += geo.i4d4(res[0], bc, -Pm*(kx**2 + ky**2), cscale = zscale)

            elif field_col == ("temperature",""):
                mat = geo.i4(res[0], bc, (kx**2 + ky**2)*(Pm**2*Ra/Pr))
...
class BoussinesqDynamoPlaneVisu(BoussinesqDynamoPlaneConfig, base_model.BaseModel):
