...
def i2x2(nr, nl, l, bc, coeff = 1.0):
    """Create a i2x2 radial operator kronecker with an identity"""

    mat = coeff*spsp.kron(rad.qid(nl,0,[0]), rad.i2x2(nr, l, bc))
    return sphbc.constrain(mat, nr, nl, bc)


def i2x2lapl(nr, nl, l, bc, coeff = 1.0):
    """Create a i2x2lapl radial operator kronecker with an identity"""

    mat = coeff*spsp.kron(rad.qid(nl,0,[0]), rad.i2x2lapl(nr, l, bc))
    return sphbc.constrain(mat, nr, nl, bc)


def i4x4(nr, nl, l, bc, coeff = 1.0):
    """Create a i4x4 radial operator kronecker with an identity"""

    mat = coeff*spsp.kron(rad.qid(nl,0,[0]), rad.i4x4(nr, l, bc))
    return sphbc.constrain(mat, nr, nl, bc)


def i4x4lapl(nr, nl, l, bc, coeff = 1.0):
    """Create a i4x4lapl radial operator kronecker with an identity"""

    mat = coeff*spsp.kron(rad.qid(nl,0,[0]), rad.i4x4lapl(nr, l, bc))
    return sphbc.constrain(mat, nr, nl, bc)


def i4x4lapl2(nr, nl, l, bc, coeff = 1.0):
    """Create a i4x4lapl2 radial operator kronecker with an identity"""

    mat = coeff*spsp.kron(rad.qid(nl,0,[0]), rad.i4x4lapl2(nr, l, bc))
    return sphbc.constrain(mat, nr, nl, bc)


def qid(nr, nl, l, qr, bc, coeff = 1.0):
