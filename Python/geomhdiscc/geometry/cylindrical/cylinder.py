...
def x1e1(nr, nz, parity, bc, coeff = 1.0, sr = 0, sz = 1):
    """Create operator for x in R and 1st derivative in Z"""

    bcr, bcz = convert_bc(bc)
    mat = coeff*spsp.kron(c1d.d1(nz, bcz, zr = sz), rad.x1(nr, parity, bcr, zr = sr))
    return cylbc.constrain(mat, nr, nz, parity, sr, sz, bc, location = 'b')
...
def i1j1x1e1(nr, nz, parity, bc, coeff = 1.0, zscale = 1.0):
    ...
    bcr, bcz = convert_bc(bc)
    mat = coeff*zscale*spsp.kron(c1d.i1d1(nz, bcz), rad.i1x1(nr, parity, bcr))
    return cylbc.constrain(mat, nr, nz, parity, 0, 1, bc)
...
def i2j2x2e1(nr, nz, parity, bc, coeff = 1.0, zscale = 1.0):
    ...
    bcr, bcz = convert_bc(bc)
    mat = coeff*zscale*spsp.kron(c1d.i2d1(nz,bcz), rad.i2x2(nr, parity, bcr))
    return cylbc.constrain(mat, nr, nz, parity, 1, 2, bc)
...
def i2j2x2lapl(nr, nz, m, parity, bc, coeff = 1.0, zscale = 1.0):
    ...
    bcr[0] = min(bcr[0], 0)
    bcz[0] = min(bcz[0], 0)
    mat = mat + zscale**2*spsp.kron(c1d.i2d2(nz,bcz), rad.i2x2(nr, parity, bcr))
    mat = coeff*mat
    return cylbc.constrain(mat, nr, nz, parity, 1, 2, bc)
...
def i4j4x4lapl(nr, nz, m, parity, bc, coeff = 1.0, zscale = 1.0):
    ...
    bcr[0] = min(bcr[0], 0)
    bcz[0] = min(bcz[0], 0)
    mat = mat + zscale**2*spsp.kron(c1d.i4d2(nz,bcz), rad.i4x4(nr, parity, bcr))
    mat = coeff*mat
    return cylbc.constrain(mat, nr, nz, parity, 2, 4, bc)
...
def i4j4x4lapl2(nr, nz, m, parity, bc, coeff = 1.0, zscale = 1.0):
    ...
    bcr[0] = min(bcr[0], 0)
    bcz[0] = min(bcz[0], 0)
    mat = mat + 2.0*zscale**2*spsp.kron(c1d.i4d2(nz,bcz), rad.i4x4laplh(nr, m, parity, bcr))
    mat = mat + zscale**4*spsp.kron(c1d.i4d4(nz,bcz), rad.i4x4(nr, parity, bcr))
    mat = coeff*mat
    return cylbc.constrain(mat, nr, nz, parity, 2, 4, bc)
