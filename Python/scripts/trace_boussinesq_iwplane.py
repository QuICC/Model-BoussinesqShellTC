"""Script to run a marginal curve trace for the rescaled Boussinesq inertial waves Boussinesq model (Toroidal/Poloidal formulation)"""

import numpy as np
import functools

import geomhdiscc.model.boussinesq_iwplane as mod
import geomhdiscc.linear_stability.marginal_curve as MarginalCurve

# Create the model and activate linearization
model = mod.BoussinesqIWPlane()
model.linearize = True
model.use_galerkin = False

# Set resolution, parameters, boundary conditions
res = [64, 0, 0]
eq_params = {'ekman':1e-7, 'scale1d':2.0, 'fast_mean':0, 'rescaled':1, 'rayleigh':0}
auto_params = model.automatic_parameters(eq_params)
for k,v in auto_params.items():
    eq_params[k] = v
bcs = {'bcType':model.SOLVER_HAS_BC, 'velocity':1}
phi = 0
kp = 1.30476
kpm = 0.02
kpp = 0.03
