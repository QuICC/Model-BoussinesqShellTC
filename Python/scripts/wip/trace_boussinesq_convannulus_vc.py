"""Script to run a marginal curve trace for the Boussinesq convection in a cylindrical annulus model"""

import numpy as np

import geomhdiscc.model.wip.boussinesq_convannulus_vc as mod

# Create the model and activate linearization
model = mod.BoussinesqConvAnnulusVC()
...
res = [10, 0, 10]
eq_params = {'prandtl':1, 'rayleigh':5.5, 'ro':1, 'rratio':0.35, 'zrratio':1.0}
eigs = [1]
...
if show_solution:
    ...
    sol_c = mod.annulus.xdiv(res[0], res[2], a, b, mod.no_bc(), sz = 0)*sol_u + 1j*eigs[0]*sol_v + mod.annulus.xe1(res[0], res[2], a, b, mod.no_bc(), sr = 0)*sol_w
