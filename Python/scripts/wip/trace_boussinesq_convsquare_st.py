"""Script to run a marginal curve trace for the Boussinesq rotating convection in a square (streamfunction formulation)"""

import numpy as np

import geomhdiscc.model.wip.boussinesq_convsquare_st as mod

# Create the model and activate linearization
model = mod.BoussinesqConvSquareST()
...
eigs = []
bc_str = 0 # 0: NS/NS, 1: SF/SF, 2: SF/NS, 3: NS/SF
...
if show_solution:
    ...
    sol_s = evp_vec[0:res[0]*res[2],mode]
    sol_t = evp_vec[res[0]*res[2]:2*res[0]*res[2],mode]
    # Extract u and w from streamfunction solution
    sol_u = mod.c2d.e1(res[0], res[2], mod.no_bc(), sx = 0)*sol_s
    sol_w = -mod.c2d.d1(res[0], res[2], mod.no_bc(), sz = 0)*sol_s
    ...
    # Compute pressure from solution
    import scipy.sparse.linalg as splin
    rhs = (eq_params['rayleigh']/16.)*mod.c2d.i2j2e1(res[0], res[2], mod.no_bc())*sol_t
