...
def i2j1d0d1(nx, nz, bc):
   """Create a quasi identity block of order 2,2"""

   bcx, bcz = convert_bc(bc)
   mat = spsp.kron(c1d.qid(nz,1,bcz), c1d.i2(nx,bcx))
...
def i2j0(nx, nz, bc):
   ...
   bcx, bcz = convert_bc(bc)
   mat = spsp.kron(c1d.qid(nz,0,bcz), c1d.i2(nx,bcx))
...
def i2j1(nx, nz, bc):
   ...
   bcx, bcz = convert_bc(bc)
   mat = spsp.kron(c1d.i1(nz,bcz), c1d.i2(nx,bcx))
   return c2dbc.constrain(mat, nx, nz, bc, 2, 1)
...
def i2j0laplh(nx, nz, k, bc):
   """Create operator for 2nd integral in x and 1st integral in z of Laplacian T_n(x)T_n(z)"""

   bcx, bcz = convert_bc(bc)
   mat = spsp.kron(c1d.qid(nz,0,bcz), c1d.i2laplh(nx,k,bcz))
   return c2dbc.constrain(mat, nx, nz, bc, 2, 0)


def i2j1laplh(nx, nz, k, bc):
   """Create operator for 2nd integral in x and 1st integral in z of Laplacian T_n(x)T_n(z)"""

   bcx, bcz = convert_bc(bc)
   mat = spsp.kron(c1d.i1(nz,bcz), c1d.i2laplh(nx,k,bcz))
   return c2dbc.constrain(mat, nx, nz, bc, 2, 1)
...
def i4j1(nx, nz, bc):
   ...
   bcx, bcz = convert_bc(bc)
   mat = spsp.kron(c1d.i1(nz,bcz), c1d.i4(nx,bcx))
   return c2dbc.constrain(mat, nx, nz, bc, 4, 1)


def i4j1d0d1(nx, nz, bc):
   """Create operator for 4th integral in x,z of T_n(x)T_n(z)"""

   bcx, bcz = convert_bc(bc)
   mat = spsp.kron(c1d.qid(nz,1,bcz), c1d.i4(nx,bcx))
   return c2dbc.constrain(mat, nx, nz, bc, 4, 1)
